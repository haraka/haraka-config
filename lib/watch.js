...
function schedule_watchCb(reader, dir) {
  ...
  debounce(dir, DIR_CALLBACK_DELAY, () => {
    ...
    if (watchers[dir]?.id) watchers[dir].id.mtimeMs = identity(dir)?.mtimeMs
    o.watchCb()
  })
}
...
function identity(target) {
  try {
    const { dev, ino, mtimeMs } = fs.statSync(target)
...
Watch.reconcile = (reader) => {
  for (const [dir, recursive] of desired(reader)) {
    ...
    if (had?.id && !same_file(had.id, now)) close_watcher(dir)
    Watch.dir(reader, dir, { recursive, quiet: true })
    const w = watchers[dir]
    if (!w) continue
    // a fresh watcher, or an entry added or removed without an event: the getDir() consumer re-reads
    if (w !== had || w.id?.mtimeMs !== now?.mtimeMs) schedule_watchCb(reader, dir)
    if (w === had && now) w.id = now
  }
  for (const [name, args] of Object.entries(reader._read_args)) {
