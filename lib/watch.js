...
// the first symlink among the components of `p`. Below the config's own
// directory every component is inspected; elsewhere the top two levels are
// not, so a symlinked /etc/haraka or /etc never puts a watcher on /etc or /
const MIN_DEPTH = 3
function first_link(p, base) {
  const inside = p.startsWith(base + path.sep)
  ...
  p.slice(root.length)
    ...
    .forEach((seg, i) => {
      ...
      if (inside ? cur.startsWith(base + path.sep) : i + 1 >= MIN_DEPTH) candidates.push(cur)
    })
...
function sources(reader, name) {
  ...
  if (fs.existsSync(name)) return found // a fallback only stands in while the file is missing
