...
function check(reader, dir) {
  for (const name of Object.keys(reader._read_args)) {
    const args = reader._read_args[name] // a reload's callback may have stopped or re-read a sibling
...
Watch.dir = (reader, dir, { recursive = false, quiet = false } = {}) => {
  ...
  try {
    ...
    watcher.on?.('error', (e) => {
      ...
      if (watchers[dir]) schedule_watchCb(reader, dir) // whatever happened while it was down
    })
    watcher.unref?.()
