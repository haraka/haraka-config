...
// the first symlink among `p` and its components below `base`. Components
// above the config's own directory are not inspected: a symlinked parent
// (/etc/haraka -> /opt/haraka/config) must not put a watcher on /etc
...
function sources(reader, name) {
  ...
  if (source && source !== name) for (const p of [source, ...links_of(source)]) found.add(p)
  return found
}
  ...
  enoent.timer = setInterval(() => {
    ...
    for (const dir of [...enoent.dirs.keys()]) {
      fs.stat(dir, (err) => {
        ...
        if (err || !opts) return
        Watch.dir(reader, dir, opts) // unqueues on success; a failure other than ENOENT stays queued
        if (watchers[dir]) Watch.reload_tracked(reader, dir)
      })
    }
