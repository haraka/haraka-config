...
function clear_timers_under(target) {
...
// a getDir() slot on the dir itself, or a tracked file directly inside it
function still_tracked(reader, dir) {
  return Object.keys(reader?._read_args ?? {}).some((name) => name === dir || path.dirname(name) === dir)
}

// Stop watching `target`, a file or a getDir() directory, and release its
// directory watcher once nothing else in that directory is tracked. Idempotent.
Watch.close = (reader, target) => {
  clear_timers_under(target)
