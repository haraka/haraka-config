...
// section/key names that would let crafted config walk into a prototype
...
exports.parseIni = (name, options = {}, data) => {
  ...
  for (let line of data.split(/\r\n|\r|\n/)) {
    ...
    if (match) {
      const sect = match[1]
      if (FORBIDDEN.has(sect)) {
        exports.logger(`Ignoring unsafe section name '[${sect}]' in config file '${name}'`)
        // discard keys until the next section, don't leak into main
        current_sect = {}
        current_sect_name = sect
        continue
      }
      if (!hasOwn(result, sect)) result[sect] = {}
      current_sect = result[sect]
      current_sect_name = sect
      continue
    }
    ...
    if (FORBIDDEN.has(baseKey)) {
      exports.logger(`Ignoring unsafe key '${keyName}' in config file '${name}'`)
      continue
    }

    const setter = this.getSetter(current_sect, isArray)
...
exports.init_booleans = (options, result) => {
  ...
  for (let i = 0; i < options.booleans.length; i++) {
    ...
    if (FORBIDDEN.has(section) || FORBIDDEN.has(key)) continue
    ...
    if (!hasOwn(result, section)) result[section] = {}
