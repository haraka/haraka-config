...
async function resolve_confined(root, entryPath) {
  const stat = await fsp.lstat(entryPath)
  if (!stat.isSymbolicLink()) return entryPath

  let target
  try {
    target = await fsp.realpath(entryPath)
  } catch (ignore) {
    return null // dangling symlink
  }

  if (!contains(root, target)) {
    console.error(`Skipping ${entryPath}: symlink target is outside ${root}`)
    return null
  }
  return target
}
...
class Reader {
  ...
  read_config(name, type, cb, options) {
    ...
    if (this.watch_files && !options?.no_watch) watch.dir(this, path.dirname(name))
    return result
  }
  ...
  async read_dir(name, opts = {}) {
    ...
    // getDir() walks whatever it finds. Keep the walk inside `name`
    const root = dirs.length ? await fsp.realpath(name) : null
    const seen = new Set(root ? [root] : [])

    for (const dir of dirs) {
      for (const entry of await fsp.readdir(dir)) {
        const entryPath = path.join(dir, entry)
        const resolved = await resolve_confined(root, entryPath)
