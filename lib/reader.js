...
class Reader {
  ...
  read_config(name, type, cb, options) {
    ...
    this._read_args[name] = { type, cb, options, fallbacks: previous?.fallbacks }
    ...
    if (this.watch_files) watch.file(this, name)
    return result
  }
  ...
  async read_dir(name, opts = {}) {
    ...
    if (opts.watchCb) watch.dir(this, name, { recursive: true })
    return contents
  }

  // the files a missing `name` is read from instead, in order of preference:
  //   *.h?json -> *.yaml    (yaml is friendlier to hand-write)
  //   <name>   -> <name>.js (lets a .js module supply config, e.g. from
  //                          process.env -- issue #39; needs HARAKA_JS_CONFIG=1)
  fallbacks(name) {
    if (/\.h?json$/.test(name)) return [name.replace(/\.h?json$/, '.yaml')]
    if (!/\.js$/.test(name) && process.env.HARAKA_JS_CONFIG) return [`${name}.js`]
    return []
  }

  // Returns { name, type }; `missing` is true when nothing exists. Used by
  // both load_config() and last_load_error() so their cache keys never drift
  _resolve_missing(name, type) {
    if (fs.existsSync(name)) return { name, type, missing: false }
    for (const alt of this.fallbacks(name)) {
      if (fs.existsSync(alt)) return { name: alt, type: types.type_of(alt), missing: false }
    }
  ...
  load_config(name, type, options) {
    ...
    // recorded even while absent, so the watcher notices a fallback appearing
    const slot = this._read_args[name]
    if (slot) slot.fallbacks = this.fallbacks(name)
