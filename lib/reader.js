...
class Reader {
  ...
  read_config(name, type, cb, options) {
    ...
    this._read_args[name] = { type, cb, options, source: previous?.source }
  ...
  load_config(name, type, options) {
    ...
    // a missing fallback keeps its slot's last source, so its return is noticed
    const slot = this._read_args[name]
    if (slot && !resolved.missing) slot.source = resolved.name
