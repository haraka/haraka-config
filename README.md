...
Haraka watches the directory containing each config file it has read, so a file that is created later is noticed as soon as it appears. If the directory itself does not exist yet, it is polled every 60 seconds and watched once it is created.
